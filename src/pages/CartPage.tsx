  ...
  if (items.length === 0) {
    return (
      <div className="py-24 text-center">
        ...
        <Link to="/products" className="inline-flex items-center gap-2 bg-rose-600 hover:bg-rose-500 text-white px-6 py-3 rounded-xl font-semibold transition-colors">
          商品を見る
        </Link>
      </div>
    );
    ...
    <div className="py-12 px-6">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6 space-y-3">
          ...
          <button className="w-full bg-rose-600 hover:bg-rose-500 text-white py-3.5 rounded-xl font-semibold transition-colors mt-2">
            レジに進む
          </button>
          <button onClick={clearCart} className="w-full text-sm text-zinc-500 hover:text-zinc-300 py-2 transition-colors">
            カートを空にする
          </button>
        </div>
      </div>
